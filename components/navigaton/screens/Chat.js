    ...
    return (
        <View className="container h-full mt-3 " >
            ...
            <View className='container px-2 flex flex-row '>
                <TextInput className='container border-[1px] align-text-top border-slate-400 h-10 rounded px-3 text-base items-center' placeholder={'Search here'} />
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => { navigation.navigate('Message', { userName: item.userName }) }} className='container' >
                        <View style={navStyles.card_border} className='flex flex-row justify-between px-2'>
                            ...
                            <View className='flex-col justify-center p-1 ml-2 w-80 border-opacity-5 border-slate-100'>
                                <View className='flex flex-row justify-between mb-1 items-center'>
                                    ...
                                    <Text className=' '>{item.messageTime}</Text>
                                </View>
                                <Text className='font-light'>{item.messageText}</Text>
                            </View>
                        </View>
                    </TouchableOpacity>
                )}
